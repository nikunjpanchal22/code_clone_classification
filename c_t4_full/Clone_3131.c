...
#define N 10
#define VERY_LARGE_INT 9999

int main (int argc, char *argv[]) {
    int ROOT = 0, i, rank, size, tag = 1234;
    int A[N], *localArray, count, localMin, globalMin;
    MPI_Init(&argc, &argv);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    count = N/size;
    localArray = (int *)calloc(count, sizeof(int));
    if (rank == ROOT) {
        for (i = 0; i < N; i++) A[i] = rand()%10;
        for (i = 0; i < size; ++i) MPI_Send(&A[i * count], count, MPI_INT, i, tag, MPI_COMM_WORLD);
