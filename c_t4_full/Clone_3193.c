int main () {
 ...
 Redefining the delim variable inside the function

int getcols (char *s, const char * delim_arg, char *** cols) {
    const char * delim = delim_arg;
    char * temp = strtok(s, delim);
    char ** cols_temp = (char **) malloc (sizeof (char *));
    int i;

    for (i = 0; temp != NULL; i++) {
        cols_temp[i] = temp;
        temp = strtok(NULL, delim);
        cols_temp = (char **) realloc (cols_temp, (i + 2) * sizeof (char *));
    }
    cols = &cols_temp;
    return i;
}
