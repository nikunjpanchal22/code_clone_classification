...
int main () {
    time_t t;
    struct tm *present_time, future_time;
    double days;
    
    time(&t);
    present_time = localtime(&t);
    
    future_time = *present_time;
    future_time.tm_mon = 11;
    future_time.tm_mday = 25;
    if (present_time->tm_mon == 11 && present_time->tm_mday > 25)
        future_time.tm_year++;
        
    days = difftime(mktime(&future_time), t) / 86400.0;
    printf("%g days\n", days);
