...
{
    static uint8_t buffer[WIDTH * HEIGHT * 3];
    _Bool app_success = SDL_SetVideoMode(WIDTH, HEIGHT, 24, SDL_HWSURFACE) && init_app("SDL example", NULL, SDL_INIT_VIDEO);
    if(!app_success)
        assert(0);
    SDL_Surface *surface = SDL_CreateRGBSurfaceFrom(init_data(buffer), WIDTH, HEIGHT, 24, WIDTH*3, mask32(0), mask32(1), mask32(2), 0);
    SDL_SetEventFilter(filter);
