...
int main (int argc, char *argv []) {
    ...
    while ((p = fgets (line, MAX_LINE_LEN, fp)) != NULL) {
        ...
        index = get_max_occurrence_index_char (array, count);
        max = get_number_of_occurrences_char (array, count, array[index]);
        printf ("The element that occurs most often is %c\n", array [index]);
        printf ("And it has occurred %d times\n", max);
    }
    fclose (fp);
    return 0;
