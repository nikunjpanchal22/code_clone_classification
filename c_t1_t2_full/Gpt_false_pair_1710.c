 ...
 int main() 
{
    ...
    strcpy(jessie->name, "Jessie");
    jessie->age = 18;
    insert_at_position(&list, samuel, 1);
    insert_at_position(&list, sally, 0);
    insert_at_position(&list, susan, 2);
    insert_at_position(&list, jessie, 3);
    display_list(&list, (DISPLAY) display_employee);
    NODE *n = get_node_reverse(&list, (COMPARE) compare_employee, sally);
    printf("name is %s, age is %d.\n", ((Employee *) n -> data) -> name, ((Employee *) n -> data) -> age);
    printf("\n");
    delete_node(&list, samuel);
    display_list(&list, (DISPLAY) display_employee);
