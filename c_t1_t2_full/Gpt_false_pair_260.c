int main (int argc, char **argv) {
 ...
 int main (int argc, char** argv) {
    ...
    FILE *template = fopen (argc > 2 ? argv[2] : "../dat/template.txt", "r");
    FILE *output = argc > 3 ? fopen (argv[3], "w") : stdout;
    if (!input || !template || !output) {
        fprintf (stderr, "error: file open failed.\n");
        return 1;
    }
    while (fgets (buf, MAXC, input)) {
        char *p;
        size_t index_num = 0;
        for (p = strtok (buf, "|"); p; p = strtok (NULL, "|\n")) {
            fulfill_sp (& s [n], p, & index_num);
        }
        if (++n == MAXS) {
            fprintf (stderr, "MAXS structs filled.\n");
            break;
