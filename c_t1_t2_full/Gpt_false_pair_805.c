...
int main (void) {
    ...
    n = strtol (buffer, &endptr_n, 10);
    if (endptr_n == buffer) {
        fputs ("No number was inserted.\n", stderr);
        return EXIT_FAILURE;
    }
    d = strtod (buffer, &endptr_d);
    if (*endptr_d == '\0' || *endptr_d == '\n') {
        if (endptr_d == endptr_n) {
            puts ("You put in a plain integer.\n");
        }
        else {
            puts ("You put in a floating point number - invalid.\n");
        }
    }
    else {
        puts ("You put in trash after the number - invalid.\n");
    }
    return EXIT_SUCCESS;
}
