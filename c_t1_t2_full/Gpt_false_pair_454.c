int main (void) {
 ...
 int main (void) {
    int arrItem [5];
    char *inputString = "14 22 33 48 5q";
    size_t numsConv;
    stringToIntList (inputString, arrItem, sizeof arrItem / sizeof arrItem [0], &numsConv);
    if (numsConv > 0) {
        size_t i;
        printf ("\nConverted %lu items from \"%s\":\n", (unsigned long) numsConv, inputString);
        for (i = 0; i < numsConv; i++)
            printf ("arrItem[%lu] = %d\n", (unsigned long) i, arrItem[i]);
    }
    else {
