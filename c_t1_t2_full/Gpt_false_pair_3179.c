...
int main () {
    const char *text = "I have a program that displays UTF-16 encoded strings " "with a size limitation (say MAX_LEN). Whenever I get a string with a " "length > MAX_LEN, I want to find out where I could split it so it " "would be printed gracefully.";
    substr_t lines [100];
    const uint32_t max_num_lines = sizeof (lines) / sizeof (lines[0]);
    const int num_lines = wrap (text, 48, lines, max_num_lines);
    if (num_lines < 0) {
