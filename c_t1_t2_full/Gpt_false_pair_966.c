int main () {
 ...
 int main () {
    char *args [2] = {"DD", NULL};
    int D, waitVal3, waitVal4, rc, term_sig;
    D = dangerous_clone (clonable_function, malloc (4096) + 4096, CLONE_FS | CLONE_FILES | CLONE_SIGHAND, NULL);
    if (D == -1) {
        printf ("fork failed\n");
        exit (1);
    }
    ...
    waitVal4 = waitpid (D, &term_sig, WCONTINUED); 
    waitVal3 = (waitVal4);
    printf ("WAIT VAL: %d", waitVal3);
    if (waitVal3 == D) {
        printf ("\nPid = %d Code AA: process Pid = %d terminated\n", getpid (), D);
