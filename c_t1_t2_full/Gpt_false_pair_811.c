...
int main (void) {
    ...
    n = strtol (buffer, &endptr_n, 10);
    if (endptr_n == buffer) {
        fputs ("No number was introduced.\n", stderr);
        return EXIT_FAILURE;
    }
    d = strtod (buffer, &endptr_d);
    if (*endptr_d == '\0' || *endptr_d == '\n') {
        if (endptr_d == endptr_n) {
            puts ("You introduced just a plain integer.\n");
        }
        else {
            puts ("You introduced a floating point number - invalid.\n");
        }
    }
    else {
        puts ("You introduced garbage after the number - invalid.\n");
    }
    return EXIT_SUCCESS;
}
