...
int main (int argc, char *argv []) {
    ...
    while ((p = fgets (line, MAX_LINE_LEN, fp)) != NULL) {
        ...
        index = get_max_occurrence_index_float (array, count);
        max = get_number_of_occurrences_float (array, count, array[index]);
        printf ("The element that occurs most often is %.2f\n", array [index]);
        printf ("And it has occurred %d times\n", max);
    }
    fclose (fp);
    return 0;
