int main (int argc, char *argv []) {
 ...
 int main (int argc, char *argv[]) {
    ...
    IplImage *img_data;
    img_data = cvCreateImageHeader(cvSize (img->width, img->height), img->depth, img->nChannels);
    if (!img) {
        printf ("Image can NOT Load!!!\n");
        return 1;
    }
    cvNamedWindow ("myfirstwindow", CV_WINDOW_FULLSCREEN);
    cvShowImage ("myfirstwindow", img);
    printf ("Height: %d\nwidth: %d\nnchannels:%d\n", img -> height, img -> width, img -> nChannels);
    cvCvtColor (img, img_data, CV_RGB2GRAY);
    cvWaitKey (0);
    cvReleaseImage (& img);
