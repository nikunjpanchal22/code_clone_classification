int main (void) {
 ...
 int main (void) {
    int elements [5];
    char *sourceString = "14 22 33 48 5q";
    size_t converted;
    stringToIntList (sourceString, elements, sizeof elements / sizeof elements [0], &converted);
    if (converted > 0) {
        size_t i;
        printf ("\nConverted %lu items from \"%s\":\n", (unsigned long) converted, sourceString);
        for (i = 0; i < converted; i++)
            printf ("elements[%lu] = %d\n", (unsigned long) i, elements[i]);
    }
    else {
