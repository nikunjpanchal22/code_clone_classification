...
int main (void) {
    ...
    n = strtol (buffer, &endptr_n, 10);
    if (endptr_n == buffer) {
        fprintf (stderr, "You didn't enter a number.\n");
        return EXIT_FAILURE;
    }
    d = strtod (buffer, &endptr_d);
    if (*endptr_d == '\0' || *endptr_d == '\n') {
        if (endptr_d == endptr_n) {
            printf ("You entered just a plain integer.\n");
        }
        else {
            printf ("You entered a floating point number - invalid.\n");
        }
    }
    else {
        printf ("You entered garbage after the number - invalid.\n");
    }
    return EXIT_SUCCESS;
}
