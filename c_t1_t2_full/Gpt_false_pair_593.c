void main () {
 ...
 void main () {
    Entities figures [NUM_ENTS] = {MakeRectangle (0, 0, 40, 40), MakeElipse (256, 128, 80, 40),
    }
    for (uint32_t x = 0; x < width; ++x) {
        for (uint32_t y = 0; y < height; ++x) {
            for (uint32_t ent_ii = 0; ent_ii < NUM_ENTS; ++ent_ii) {
                if (figures[ent_ii]->PositionIncludes(x, y)) {
                    image[x][y] = 1;
                    break;
                }
            }
