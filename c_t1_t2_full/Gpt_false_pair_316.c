    ...
    while (n > 0) {
        ...
        while (fgets (line + offset, current_buffer_size, stdin)) {
            if (line[strlen (line) - 1] == '\n') {
                ...
                int increased_buffer_size = current_buffer_size * 2;
                char *tmp = (char *) realloc (line, increased_buffer_size);
                if (tmp) {
                    line = tmp;
                    offset = current_buffer_size - 1;
                    current_buffer_size = increased_buffer_size;
                }
                else {
                    break;
                }
        ...
        errno = 0;
        dividend = (unsigned long int) strtoul (line, &endp, 10);
        if (endp != line) {
            if (dividend % divisor == 0)
                answer++;
        }
