...
int main (void) {
    ...
    n = strtol (buffer, &endptr_n, 10);
    if (endptr_n == buffer) {
        fputs ("You didn't give a number.\n", stderr);
        return EXIT_FAILURE;
    }
    d = strtod (buffer, &endptr_d);
    if (*endptr_d == '\0' || *endptr_d == '\n') {
        if (endptr_d == endptr_n) {
            puts ("You gave just an integer.\n");
        }
        else {
            puts ("You gave a floating point number - invalid.\n");
        }
    }
    else {
        puts ("You gave rubbish after the number - invalid.\n");
    }
    return EXIT_SUCCESS;
}
