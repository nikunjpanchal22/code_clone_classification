...
int main(int argc, char *argv[])
{
    (void)argc, (void)argv;
    static uint8_t buffer[WIDTH * HEIGHT * 3];
    _Bool ok = init_app("main application", NULL, SDL_INIT_VIDEO) && SDL_SetVideoMode(WIDTH, HEIGHT, COLOR_DEPTH, SDL_HWSURFACE);
    SDL_Surface *pixels = SDL_CreateRGBSurfaceFrom(init_data(buffer), WIDTH, HEIGHT, COLOR_DEPTH, WIDTH*3, mask32(0), mask32(1), mask32(2), 0);
    SDL_SetEventFilter(filter);
