...
def main():
    ...
    principle = Decimal(input('Please enter your loan amount:'))
    rate = Decimal(input('Please enter rate of interest (percent):')) / 100
    term = Decimal(input('Please enter loan period (years):')) * 12
    interest = (principle * rate).quantize(Decimal('.01'), rounding=ROUND_HALF_EVEN)
    balance = principle + interest
    payment = (balance / term).quantize(Decimal('.01'), rounding=ROUND_CEILING)
    print("Payment\t\tAmount Paid\t\tRem.Bal.")
    for count in range(1 + term) :
        if count == 0 :
            print(count, "\t\t0.00\t\t\t", balance)
