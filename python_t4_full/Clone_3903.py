...
def __init__(self, guide, *args, **kwargs):
    ...
    new_fields = OrderedDict()
    for tier, tasks in guide.tiers().items():
        questions = [(task['task'], task['question']) for task in tasks if 'question' in task]
        new_fields[tier.lower()] = forms.MultipleChoiceField(label=tier, widget=forms.CheckboxSelectMultiple(), 
                                                     choices=questions, help_text='desired set of site features')
    new_fields['name'] = self.fields['name']
    new_fields['email'] = self.fields['email']
