...
def backupToZip(folder):
    ...
    for foldername, subfolders, filenames in os.walk(folder):
        backupZip.write(foldername, arcname = os.path.relpath(foldername, folder))
        for filename in filenames:
            backupZip.write(os.path.join(foldername, filename), arcname=os.path.relpath(os.path.join(foldername, filename), folder))
