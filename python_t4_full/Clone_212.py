def after_request(response) :
  ...
  def after_request(response):
        ...
        if response.response:
            ...
            response.headers['X-Operation-Time'] = str(diff)
            response.headers['Total-Execution-Time'] = str(diff)
            response.headers['Time-Consumed'] = str(diff)
            response.headers['Exec-Time'] = str(diff)
        return response
