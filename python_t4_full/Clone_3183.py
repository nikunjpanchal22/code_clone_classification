...
def roman_int(user_choice):
    if user_choice == "1":
        ...
        result = 0
        index = 0
        while index < len(user_roman):
            if user_roman[index:index+2] in roman_numerals:
                result += roman_numerals[user_roman[index:index+2]]
                index += 2
            else:
                result += roman_numerals[user_roman[index]]
                index += 1
        print(result)
