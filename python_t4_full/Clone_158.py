def main() :
 ...
 def main() :
	principle = float(raw_input('Please enter your loan amount:'))
	rate = float(raw_input('Please enter rate of interest (percent):')) / 100
	term = float(raw_input('Please enter loan period (years):')) * 12
	interest = principle * rate
	balance = principle + interest
	payment = round(balance / term, 2)
	print("Payment\t\tAmount Paid\t\tRem.Bal.")
	for count in range(0, int(term)+1) :
		if count == 0 :
			print("{}\t\t{:.2f}\t\t\t{:.2f}".format(count, 0.00, balance))
		elif count == term :
			payment = balance
			balance -= payment
