...
def token_list_count(df) :
	df['lista_count'] = df['tokens'].apply(lambda tokens: sum(tok in seta for tok in tokens))
	df['listb_count'] = df['tokens'].apply(lambda tokens: sum(tok in setb for tok in tokens))
