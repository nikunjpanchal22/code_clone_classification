...
def dashboard(request):
    ...
    sightings = Sighting.objects.filter(IMS_plant=form.cleaned_data['selectedplant']) if form.is_valid() else Sighting.objects.all().order_by('date')
    return render(request, 'dashboard.html', context={'sightings': sightings, 'form': form})
