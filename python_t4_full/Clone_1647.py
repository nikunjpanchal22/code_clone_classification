def age_in_years(from_date, to_date = datetime.date.today()) :
 ...
 def age_in_years_array(from_date, to_date=datetime.date.today()):
	if (DEBUG):
		print("Calculate the age in years from date "+from_date+" to date "+to_date)
	date_diff = [from_date.year-to_date.year, from_date.month, to_date.month, from_date.day, to_date.day]
	if date_diff[1] > date_diff[2] and date_diff[4] > date_diff[3]:
		date_diff[0] -= 1
