def __init__(self, ** kwargs) :
  ...
  def __init__(self, **kwargs):
    ...
    for k,v in self.relationships.items():
        need.update(k)
    need = need.intersection(kwargs.keys())
    derive = set()
    for p in itertools.permutations(need):
        if p in self.relationships:
            derive.add(self.relationships[p])
    underivable_keys = set(self.relationships.values()) - derive - set(kwargs)
