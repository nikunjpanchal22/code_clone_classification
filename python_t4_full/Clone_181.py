...
def roman_numeral(user_choice):
    ...
    for i in range(len(user_choice)):
        for j in range(len(roman_numerals)):
            if user_choice[index:index+len(roman_numerals[j][0])] == roman_numerals[j][0]:
                result += roman_numerals[j][1]
                index += len(roman_numerals[j][0])
                break
