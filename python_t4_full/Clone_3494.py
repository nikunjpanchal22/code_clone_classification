...
def dashboard(request):
    ...
    sightings = Sighting.objects.filter(IMS_plant=form.cleaned_data.get('selectedplant')) if form.is_valid() else Sighting.objects.all().order_by('date')
    return render(request, 'dashboard.html', {'sightings': sightings, 'form': form})
