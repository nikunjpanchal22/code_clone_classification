public Message WrapB (int b, int millisecondsTimeout) {
  ...
  public Message WrapB (int b, int millisecondsTimeout) {
    int count = 0;
    while (Interlocked.CompareExchange (ref pendingB, b, null) != null) {
        Thread.Yield (); 
        if (count > millisecondsTimeout) {
            return new Message (null, b);
        }
    count++;
    }
    while (Interlocked.CompareExchange (ref pendingB, null, null) == b) {
        Thread.Yield (); 
        if (count > millisecondsTimeout) {
            int payload = Interlocked.CompareExchange (ref pendingB, null, b);
            return payload == b ? new Message (null, b) : null;
        }
    count++;
    }
