public static void ReplaceTextInWordDoc (Object findMe, Object replaceMe, ApplicationClass app) {
 ...
 public static void ReplaceTextInWordDoc (Object findMe, Object replaceMe, ApplicationClass app) {
    using (WordprocessingDocument doc = WordprocessingDocument.Open (app.Application.ActiveDocument.Path, true))
    {
        Regex regexText = new Regex(findMe.ToString());
        foreach (Text text in doc.MainDocumentPart.Document.Descendants ().OfType<Text>())
        {
            if (regexText.IsMatch(text.Text))
            {
                text.Text = replaceMe.ToString();
            }
        }
    } 
}
