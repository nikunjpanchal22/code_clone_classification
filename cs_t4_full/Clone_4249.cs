public DataTable GetExcelDataTable (string fileName) {
  ...
  public DataTable GetExcelDataTable (string location) {
    string connectionString = Path.GetExtension (location) == ".xls" ? 
    string.Format ("Provider=Microsoft.Jet.OLEDB.4.0;Data source={0}; Extended Properties=Excel 8.0;", location): 
    string.Format ("Provider=Microsoft.ACE.OLEDB.12.0;Data Source={0}; Extended Properties=Excel 12.0;", location); 
    var conn = new OleDbConnection (connectionString);
    var com = new OleDbCommand("SELECT * FROM [Sheet1$];", conn); 
    var adapter = new OleDbDataAdapter (com); 
    var dt = new DataTable (); 
    conn.Open(); 
    adapter.Fill(dt);
    conn.Close();
