...
private bool Matches (object expectedValue, object actualValue) {
    ...
    if (eq.Matches (actualValue) || MatchesVisited(expectedValue, actualValue, ref matches) || MatchesDictionary(expectedValue, actualValue, ref matches) || MatchesList(expectedValue, actualValue, ref matches) || MatchesType(expectedValue, actualValue, ref matches) || MatchesPredicate(expectedValue, actualValue, ref matches) || MatchesFields(expectedValue, actualValue, eq, ref matches)) {
        return true;
    }
    return false;
}
