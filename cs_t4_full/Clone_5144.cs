 ...
 protected Dictionary<DateTime, String> getDateRange(String lowerDate, String higherDate, String frequency)
{
    ...
    while (startDate < endDate)
    {
        if(frequency.Equals("weekly"))
        {
            ...
            returnDict.Add(startDate, startDate.ToString("MMM d") + "–" + tempDate.ToString("MMM d"));
            startDate = tempDate.AddDays(1);
        }
        else if(frequency.Equals("monthly"))
        {
            tempDate = startDate.AddMonths(1);
            returnDict.Add(startDate, startDate.ToString("MMMM") + " - " + tempDate.ToString("MMMM"));
            startDate = tempDate.AddDays(1);
        }
    }
    ...
    {
        returnDict.Add( startDate, startDate.ToString("MMMM d") + " - " + endDate.ToString("MMMM d"));
    }
    return returnDict;   
}
