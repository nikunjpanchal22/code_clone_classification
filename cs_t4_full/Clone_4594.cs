...
private Coordinate Calculate(Coordinate location1, Coordinate location2, Coordinate location3, Coordinate location4){    
    int maxLatitude = max(location2.Latitude, location4.Latitude);
    int minLatitude = min(location2.Latitude, location4.Latitude);
    double maxLongitude;
    double minLongitude;
    Random random = new Random(DateTime.Now.Millisecond);
    Coordinate randomCoordinate = new Coordinate(){Latitude = random.Next(minLatitude, maxLatitude)};
    if (randomCoordinate.Latitude > location1.Latitude){
