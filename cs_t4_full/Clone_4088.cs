...
private bool Matches (object expectedValue, object actualValue) {
    ...
    if (MatchesNull (expectedValue, actualValue, ref matches) && !matches) {
        Constraint eq = new EqualsConstraint (expectedValue).Within (tolerance ?? _regionalTolerance);
        matches = eq.Matches (actualValue);

        if (MatchesVisited (expectedValue, actualValue, ref matches) && matches) {
            matches = MatchesDictionary (expectedValue, actualValue, ref matches);
            matches = MatchesList (expectedValue, actualValue, ref matches);
            matches = MatchesType (expectedValue, actualValue, ref matches);
            matches = MatchesPredicate (expectedValue, actualValue, ref matches);
