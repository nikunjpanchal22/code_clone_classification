...
public virtual bool Equals (EntityBase other) {
    ...
    if (ReferenceEquals (other, null) || ReferenceEquals (other, this)) return false;

    if (!NHibernateProxyHelper.GetClassWithoutInitializingProxy (other).Equals(NHibernateProxyHelper.GetClassWithoutInitializingProxy (this)))
        return false;
    
    if (Equals (other.Id, 0) || Equals (Id, 0)) return false;
