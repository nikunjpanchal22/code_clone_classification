 ...
 protected void btnNext_Click1 (object sender, EventArgs e) 
{
    ...
    if (txtNextStep.Tag == null)
    {
        dt = eobj.GetTicketFirstUpdate(Convert.ToString(txtSupportRef.Text));
    }
    else
    {
        dt = eobj.GetNextTicketUpdate(Convert.ToString(txtSupportRef.Text), Convert.ToString(txtNextStep.Tag));
    }
    if (dt.Rows.Count != 0)
    {
