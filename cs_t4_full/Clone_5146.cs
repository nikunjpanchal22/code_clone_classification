 ...
 protected Dictionary<DateTime, String> getDateRange(String lowerDate, String higherDate, String frequency){
    ...
    while (startDate < endDate)
    {
        if(frequency.Equals("weekly"))
        {
            ...
            returnDict.Add(startDate, startDate.ToString("yyyy'-'MM'-'dd") + " to " + tempDate.ToString("yyyy'-'MM'-'dd"));
            startDate = tempDate.AddDays(1);
        }
        else if(frequency.Equals("monthly"))
        {
            tempDate = startDate.AddMonths(1);
            returnDict.Add(startDate, startDate.ToString("MMMM yyyy") + " to " + tempDate.ToString("MMMM yyyy"));
            startDate = tempDate.AddDays(1);
        }
    }
    ...
    {
        returnDict.Add( startDate, startDate.ToString("yyyy'-'MM'-'dd") + " to " + endDate.ToString("yyyy'-'MM'-'dd"));
    }
    return returnDict;   
}
