  ...
  protected Dictionary<DateTime, String> getDateRange(String lowerDate, String higherDate, String frequency){
    ...
    while (startDate < endDate)
    {
        if(frequency.Equals("weekly"))
        {
            ...
            returnDict.Add(startDate, startDate.ToString("ddd, MM/dd/yyyy") + " - " + tempDate.ToString("ddd, MM/dd/yyyy"));
            startDate = tempDate.AddDays(1);
        }
        else if(frequency.Equals("monthly"))
        {
            tempDate = startDate.AddMonths(1);
            returnDict.Add(startDate, startDate.ToString("MMMM dd, yyyy") + " - " + tempDate.ToString("MMMM dd, yyyy"));
            startDate = tempDate.AddDays(1);
        }
    }
    ...
    if (startDate == endDate)
    {
        ...
        returnDict.Add( startDate, MonthAndDay[0] + startDate.Day.ToString("###") + ", " + startDate.ToString("yyyy") + " - "  + MonthAndDay[1] + endDate.Day.ToString("###") + ", " + endDate.ToString("yyyy"));
    }
    return returnDict;   
}
