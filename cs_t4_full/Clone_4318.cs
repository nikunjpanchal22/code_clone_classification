...
{ 
    ...
    if (Object.Equals(sourceType, destinationType)) 
    { 
        return true; 
    } 
    // check if the sourceType is assignable from 
    // the destinationType, return true if so 
    if (sourceType.IsAssignableFrom(destinationType)) 
    { 
        return true; 
    } 
    // check if sourceType implementes any of 
    // the interfaces of the destinationType, 
    // return true if so 
    if (destinationType.GetInterfaces().Any(t => t == sourceType)) 
    { 
        return true; 
    } 
