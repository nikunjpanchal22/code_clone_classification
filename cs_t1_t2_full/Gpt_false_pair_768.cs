private void myBtn_Click (object sender, EventArgs e) {
 ...
 private void myBtn_Click (object sender, EventArgs e) {
    if (dataGridView1.SelectedRows.Count > 0) {
        ...
        DataGridViewRow row = myDGV.Rows [rowIndex];
        Point formLoc = this.Location;
        Point gridLoc = myDGV.Location;
        Point rowLoc = myDGV.GetRowDisplayRectangle (rowIndex, false).Location;
        newForm form = new newForm ();
        form.StartPosition = FormStartPosition.Manual;
        form.Location = GetPopupStartPoint (new Point [] {formLoc, gridLoc, rowLoc}, row.Height);
        form.Show (this);
    }
}
