static void Main () {
  ...
  static void Main () {
    string strn = "A great things are happen with great humans.";
    System.Console.WriteLine ("'{0}'", strn);
    bool case1 = strn.StartsWith ("A great", System.StringComparison.CurrentCulture);
    System.Console.WriteLine ("Starts with 'A great'? {0} (with the current culture)", case1);
    bool case2 = strn.EndsWith ("humans.");
    System.Console.WriteLine ("Ends with 'humans.'? {0}", case2);
    int start = strn.IndexOf ("things");
    int end = strn.LastIndexOf ("with");
    string strn2 = strn.Substring (start, end - start);
    System.Console.WriteLine ("Between two 'things' and 'with' words: '{0}'", strn2);
}
