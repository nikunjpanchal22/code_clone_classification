...
public byte [] OpenAndCombineType4 (IList < byte [] > documents) {
    MemoryStream mainStream = new MemoryStream ();
    mainStream.Write (documents [0], 0, documents [0].Length);
    mainStream.Position = 0;
    int pointer = 1;
    ...
    try {
        ...
        throw new OfficeMergeToolException (string.Format (CultureInfo.CurrentCulture, "Error while combining files. Document index {0}", pointer), oxmle);
    }
    catch (Exception e) {
        throw new OfficeMergeToolException (string.Format (CultureInfo.CurrentCulture, "Error while combining files. Document index {0}", pointer), e);
    }
    finally {
        ret = mainStream.ToArray ();
        mainStream.Close ();
