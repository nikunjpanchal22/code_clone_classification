public bool doFindSum (ref int [] nums, int index, int current, int target) {
 ...
 public bool doFindSum (ref int [] nums, int index, int current, int target) {
    ...
    bool subtract = doFindSum (ref nums, index + 1, current - nums [index + 1], target);
    bool comparison = doFindSum (ref nums, index + 1, current > nums[index + 1], target);
    return subtract || comparison;
}
