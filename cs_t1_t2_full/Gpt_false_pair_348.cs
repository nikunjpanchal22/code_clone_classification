...
public static SqlBytes BinaryCompress (SqlBytes input)
{
    ...
    using (MemoryStream result = new MemoryStream ())
    {
        using (LZMACompressorStream lzmaCompressorStream = new LZMACompressorStream (result, CompressionMode.Compress, true))
        {
            lzmaCompressorStream.Write (input.Buffer, 0, input.Buffer.Length);
            lzmaCompressorStream.Flush ();
            lzmaCompressorStream.Close ();
        }
        return new SqlBytes (result.ToArray ());
    }
}
