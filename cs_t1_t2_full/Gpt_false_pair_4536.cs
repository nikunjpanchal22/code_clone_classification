...
private static string FindNextUniqueFilename (string fileName, Func < string, bool > inUse) {
    if (! inUse (fileName)) {
        return fileName;
    }
    var name = Path.GetFileNameWithoutExtension (fileName);
    ...
    throw new Exception (string.Format ("Too many files with the same name. Limit: {0}", maxFiles));
}
