 ...
 static int Main (string [] args)
{
    ...
    } Console.WriteLine ();
    {
        ...
        for (int i = 0; i < LoopCount; i ++) {
            ...
            int signBit = (difference >> (8*sizeof(int)-1)) & 0xff;
            maxValue =  (num1*(1-signBit)) + (num2*signBit);  
        }
        stopwatch.Stop ();
        Console.WriteLine ("Shift & Multiplication Operators: " + stopwatch.Elapsed.TotalSeconds + " seconds");
        Console.WriteLine ("Relative: " + (stopwatch.Elapsed.TotalSeconds - controlTotalSeconds) + " seconds");
    } Console.WriteLine ();
    {
        var stopwatch = new Stopwatch ();
        stopwatch.Start ();
        for (int i = 0; i < LoopCount; i ++) {
            maxValue = num1 + num2 - Math.Min (num1, num2); 
        }
        stopwatch.Stop ();
        Console.WriteLine ("Subtraction & Math.Min(): " + stopwatch.Elapsed.TotalSeconds + " seconds");
        Console.WriteLine ("Relative: " + (stopwatch.Elapsed.TotalSeconds - controlTotalSeconds) + " seconds");
    } Console.ReadLine ();
    return maxValue;
}
