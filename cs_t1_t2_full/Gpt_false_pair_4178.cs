...
private static void SetIEVersioneKeyforWebBrowserControl (string appName, int ieval) {
    ...
    try {
        ...
        FindAppkey = Convert.ToString (Regkey.GetValue (appName));
        if (FindAppkey == "" + ieval)
            YukLoggerObj.logInfoMsg ("Application FEATURE_BROWSER_EMULATION changed to " + ieval + "; changes to be seen at application restart");
        else
            YukLoggerObj.logWarnMsg ("Application FEATURE_BROWSER_EMULATION setting failed; active value is "+ ieval);
    }
    catch (Exception ex) {
        YukLoggerObj.logErrorMsg ("Application FEATURE_BROWSER_EMULATION setting failed; " + ex.Message);
    }
    finally {
        if (Regkey != null)
            Regkey.Close ();
