...
void btnApplica_Click_Scelta_Campi_Oggetto (object sender, EventArgs e) {
    Debug.Print ("btnApplica_Click_Scelta_Campi_Oggetto START");
    string id;
    if (sender.GetType () == typeof (ImageButton)) {
        ImageButton button = (ImageButton) sender;
        id = button.ID;
    ...
    SPWeb contextWeb = SPContext.Current.Web;
    string url = contextWeb.Url;
    string link = url + "/ARXEIA WEBPART/Carica documento.aspx?mode=scelta_campi_facoltativi_oggetto&obj=" + obj;
    SPUtility.Redirect (link, SPRedirectFlags.Default, Context);
}
