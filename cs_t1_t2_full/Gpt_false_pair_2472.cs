public static void DrawNormalizedAudio (ref float [] data, PictureBox pb, Color color) {
 ...
 public static void DrawNormalizedAudio (ref float [] data, PictureBox pb, Color color) {
    ...
    int width = bmp.Width - (2 * BORDER_PADDING);
    int height = bmp.Height - (2 * BORDER_PADDING);
    using (DrawingContext dc = bmp.RenderOpen())
    {
        ...
        for (int iPixel = 0; iPixel < width; iPixel ++) {
            ...
            int yMax = BORDER_PADDING + height - (int) ((max + 1) *.5 * height);
            int yMin = BORDER_PADDING + height - (int) ((min + 1) *.5 * height);
            dc.DrawLine (pen, iPixel + BORDER_PADDING, yMax, iPixel + BORDER_PADDING, yMin);
        }
        dc.Close  (); 
    }
    pb.Image = bmp;
