    ...
    using (OleDbConnection cn = new OleDbConnection (csv.ConnectionString))
    {
        ...
        using (OleDbCommand cmd = cn.CreateCommand ())
        {
            ...
            using (OleDbDataReader reader = cmd.ExecuteReader (CommandBehavior.CloseConnection))
            {
                ...
                int fieldScore = reader.GetOrdinal ("score");
                while (reader.Read ()) {
                    stats.Add (new Stat {Sport = reader.GetString (fieldSport), Date = reader.GetDateTime (fieldDate), TeamOne = reader.GetString (fieldTeamOne), TeamTwo = reader.GetString (fieldTeamTwo), Score = reader.GetInt32 (fieldScore)});
                }
            }}} foreach (Stat stat in stats) {
        Console.WriteLine ("Sport: {0}", stat.Sport);
    }
