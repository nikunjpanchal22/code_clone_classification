...
{
    ProcessOutputReaderWorkerThreadArguments args = null;
    try
    {
        args = (ProcessOutputReaderWorkerThreadArguments)obj;
    }
    catch
    {
        return;
    ...
    try
    {
        char[] reader = new char[args.ReadBufferSize];
        while (!args.Exit)
        {
            if (args.Process == null)
            {
            ...
            int readBytes = args.Process.StandardOutput.Read(reader, 0, reader.Length);
            args.IntermediateDataStore.Append(reader, 0, readBytes);
            FireOnDataRead(new String(reader, 0,readBytes));
        }
    }
    catch (ThreadAbortException)
    {
