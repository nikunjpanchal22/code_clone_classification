...
static void Method (string [] instructions) {
    Console.Clear ();
    Console.ForegroundColor = ConsoleColor.Gray;
    Dictionary < string, string [] > symbols = new Dictionary < string, string [] > ();
    symbols.Add ("ahri", new string [] {"Academy", "Challenger", "Dynasty", "Foxfire", "Midnight", "Popstar"});
    symbols.Add ("leesin", new string [] {"Traditional", "Acolyte", "Dragon Fist", "Musy Thai", "Pool Party", "SKT T1", "Knockout"});
    Console.WriteLine ("Conor's Random League of Legends Symbol Selector v0.1\r\n\r\n");
    Console.WriteLine ("What champion would you like to select a symbol for? \r\nPress enter for a random champion...    ");
    var champion = Console.ReadLine ();
    var ran = new Random ();
    if (champion.Equals (string.Empty)) {
        var tmpList = Enumerable.ToList (symbols.Keys);
        champion = tmpList [ran.Next (tmpList.Count)];
    } else {
        champion = champion.Trim ().ToLower ();
    }
    Console.Write (string.Format ("Champion {0} Selected \r\n", champion));
    if (symbols.ContainsKey (champion)) {
        Console.WriteLine (string.Format ("Your random symbol for {0} is: {1}\r\n", champion, symbols [champion] [ran.Next (symbols [champion].Length)]));
    } else {
        Console.Clear ();
        Method (instructions);
    }
