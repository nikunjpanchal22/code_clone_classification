  ...
  public ActionResult Index(int? page) 
    {
        ...
        int totalCount = 0;
        List<Order> orders = orderManage.GetOrderPage(pageNum, 5, out totalCount);
        List<OrderViewModel> orderViews = new List<OrderViewModel>();
        foreach(Order order in orders)
        {
            orderViews.Add(orderManage.GenerateOrderViewModel(order));
        }
        IPagedList<OrderViewModel> pageOrder = new StaticPagedList<OrderViewModel>(orderViews, pageNum + 1, 5, totalCount);
