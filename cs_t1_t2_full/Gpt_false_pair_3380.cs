...
public static void d (Exception e) {
    try {
        ...
        for (int i = 0; i < 3; i ++) {
            ...
            int colNum = frame.GetFileColumnNumber ();
            string className = ExtractBracketed (frame.GetMethod ().ReflectedType.FullName);
            Log.WriteLine (ThreadAndDateInfo + "Exception: " + className + "." + methodName + ", Ln " + lineNum + " Col " + colNum + ": " + e.Message);
            if (lineNum + colNum > 0)
                break;
        }
    }
    catch (Exception ee) {
        Log.WriteLine ("Tracing exception in d(Exception e)" + ee.Message);
    }
}
