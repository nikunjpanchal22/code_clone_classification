...
protected static bool TestaIntegracaoErpMigplus_V4 () {
    bool ret = true;
    try {
        string connectionStringMigplus = WebConfigurationManager.AppSettings["ConnectionStringMigplus"].ConnectionString;
        using (SqlConnection Conn = new SqlConnection(connectionStringMigplus))
        {
            Conn.Open();
        }}
