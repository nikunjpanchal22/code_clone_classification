private void myBtn_Click (object sender, EventArgs e) {
 ...
 private void myBtn_Click (object sender, EventArgs e) {
    if (dataGridView1.SelectedRows.Count > 0) {
        var rowIndex = myDGV.SelectedRows [0].Index;
        var row = myDGV.Rows [rowIndex];
        var formLocVal = this.Location;
        var gridLocVal = myDGV.Location;
        var rowStartLoc = myDGV.GetRowDisplayRectangle (rowIndex, false).Location;
        newForm form = new newForm ();
        form.StartPosition = FormStartPosition.Manual;
        form.Location = GetStartingLocation (new Point [] {formLocVal, gridLocVal, rowStartLoc}, row.Height);
        form.Show (this);
    }
}
