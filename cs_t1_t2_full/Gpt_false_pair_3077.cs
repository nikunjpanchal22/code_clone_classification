...
[HttpGet] [Route("api/TokenCancellationApi/BeginLongProcess/{seconds}")] public string BeginLongProcess(int seconds) {
    ...
    {
        if (null != cTokenSource) {
            return "A long running process is already in progress.";
        }
        cTokenSource = new CancellationTokenSource();
    } try {
        LongRunningFunc(cTokenSource.Token, seconds);
        ...
        return "The long running process has been cancelled.";
    }
    catch (Exception ex) {
        _lastError = ex.Message;
        return ex.Message;
    ...
    return "The long running process has ended successfully!";
}
