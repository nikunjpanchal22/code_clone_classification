public List < string > Keyword_Search (HtmlNode nSearch) {
 ...
 public List<string> Keyword_Search(HtmlNode node)
{
    List<string> wordsFound = new List<string>();
    string innerHtml = node.InnerHtml;
    string pattern = "(\\b" + string.Join("\\b)|(\\b", _keywordList) + "\\b)";
    Regex regExpression = new Regex(pattern, RegexOptions.IgnoreCase);
    MatchCollection matches = regExpression.Matches(innerHtml);
