...
private static AssemblyName GetAssemblyName (string source, bool isFile) {
    ...
    try {
        ...
        string ErrorFormatString = "An error occurred when calling the utility method 'GetAssemblyNameOrThrowException'.\n" + "Arguments passed in:\n" + "=> Source:\n[{0}]\n" + "=> isFile = {1}\n" + "See inner exception(s) for more detail.";
        throw new Exception (string.Format (ErrorFormatString, source, isFile), err);
    }
    if (asmName == null)
        throw new InvalidOperationException (string.Format("No Assembly Name object was retrieved from the source {0}.", source));
    return asmName;
}
