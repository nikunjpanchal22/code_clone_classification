...
private static void Main (string [] args) {
    _signalFromClient = new EventWaitHandle (true, EventResetMode.AutoReset, NameThatClientKnows);
    _tcpServer = new Socket (AddressFamily.InterNetworkV6, SocketType.Dgram, ProtocolType.Tcp);
    _tcpServer.Bind (new IPEndPoint (IPAddress.IPv6Any, 0));
    _tcpServer.Listen (3);
    var asyncOpInfo = new SocketAsyncEventArgs ();
    asyncOpInfo.Completed += CompletedConnectRequest;
    _tcpServer.AcceptAsync (asyncOpInfo);
    Console.WriteLine ("Client will attempt to connect now.");
    Console.ReadLine ();
}
