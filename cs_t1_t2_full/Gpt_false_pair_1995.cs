private void Interval (object sender, EventArgs e) {
 ...
 private void Interval (object sender, EventArgs e) {
    // Check if the 'Pause' checkbox is checked
    if (cbPause.Checked) {
        // Generate a random line
        randomLine = random.Next (lbMessage.Items.Count);
        // Enable the timer
        tmrSpace.Enabled = true;
        // Update the whenStart boolean
        if (whenStart)
            tickCount ++;
        else
            whenStart = true;
        ...
        if (cbRandomLine.Checked) {
            ...
            // Check if the tickCount is less than the number of items in the list
            if (tickCount < lbMessage.Items.Count) {
    ...
    // Check if the tickCount equals the number of items in the list
    if (tickCount == lbMessage.Items.Count)
        // Reset the tickCount
        tickCount = 0;
    // Set the Interval
