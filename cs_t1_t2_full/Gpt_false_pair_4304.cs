...
private void OnSizeChanged (object sender, EventArgs e) {
    ...
    while (HitTest (_offsetX, Height / 3).HitArea != HitArea.Date) {
        _offsetX ++;
    }
    _dayBoxWidth = 0;
    DateTime dt1 = HitTest (Width / 2, _offsetY).Time;
    while (HitTest (Width / 3, _offsetY + _dayBoxHeight).Time == dt1) {
        _dayBoxHeight ++;
    }
    _dayBoxWidth = 0;
    DateTime dt2 = HitTest (_offsetX, Height / 3).Time;
    while (HitTest (_offsetX + _dayBoxWidth, Height / 4).Time == dt2) {
        _dayBoxWidth ++;
    }
}
