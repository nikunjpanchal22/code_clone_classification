public static IEnumerable < T > MergePreserveOrder4 < T, TOrder > (this IEnumerable < IEnumerable < T > > aa, Func < T, TOrder > orderFunc) where TOrder : IComparable < TOrder > {
 ...
 public static IEnumerable < T > MergePreserveOrder4 < T, TOrder > (this IEnumerable < IEnumerable < T > > aa, Func < T, TOrder > orderFunc) where TOrder : IComparable < TOrder > {
   var items = aa.Select (xx => xx.GetEnumerator ()).Where (ee => ee.MoveNext ()).Select (ee => Tuple.Create (orderFunc (ee.Current), ee)).OrderBy (ee => ee.Item1).ToQueue ();
   while (items.Count > 0) {
       yield return items.Dequeue().Item2.Current;
       ...
       if (next.Item2.MoveNext ()) {
           ...
           if (ii == items.Count)
               items.Enqueue (Tuple.Create (value, next.Item2));
       } else
           next.Item2.Dispose ();
   }   
}
