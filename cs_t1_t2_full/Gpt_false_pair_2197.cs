private void OnSizeChanged (object sender, EventArgs e) {
 ...
 private void OnSizeChanged (object sender, EventArgs e){
    ...
    _dayBoxHeight = 0; 
    DateTime startPoint = HitTest (Width / 2, _offsetY).Time;
    while (HitTest (Width / 2, _offsetY + _dayBoxHeight).Time == startPoint){
        _dayBoxHeight++;
    }
    _dayBoxWidth = 0; 
    DateTime endPoint = HitTest (_offsetX, Height / 2).Time; 
    while (HitTest (_offsetX + _dayBoxWidth, Height / 2).Time == endPoint){
        _dayBoxWidth++;
    }
}
