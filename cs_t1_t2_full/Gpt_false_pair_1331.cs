static void Main (string [] args) {
 ...
 static void Main (string [] args) {
    ...
    using (PostgreSQLConnection cn = new PostgreSQLConnection (csv.ConnectionString))
    {
        cn.Open ();
        using (PostgreSQLCommand cmd = cn.CreateCommand ())
        {
            cmd.CommandText = "SELECT * FROM [Stats.csv]";
            cmd.CommandType = CommandType.Text;
            using (PostgreSQLDataReader reader = cmd.ExecuteReader (CommandBehavior.CloseConnection))
            {
                int fieldSport = reader.GetOrdinal ("sport");
                int fieldDate = reader.GetOrdinal ("date");
                int fieldTeamOne = reader.GetOrdinal ("teamone");
        ...
        FlutterBinding.instance.addPostFrameCallback((_) => {print(stat.Sport);});
    }
}
