...
static void Main (string [] args) {
    var sourceFileNames = new List < FileList > ();
    sourceFileNames.Add (new FileList {FileNames = "a.txt"});
    sourceFileNames.Add (new FileList {FileNames = "b.txt"});
    sourceFileNames.Add (new FileList {FileNames = "c.txt"});
    sourceFileNames.Add (new FileList {FileNames = "d.txt"});
    List < FileList > destinationFileNames = new List < FileList > ();
    destinationFileNames.Add (new FileList {FileNames = "a.txt"});
    destinationFileNames.Add (new FileList {FileNames = "d.txt"});
    var except = sourceFileNames.Except (destinationFileNames);
    foreach (var f in except)
        Console.WriteLine (f.FileNames);
    Console.ReadLine ();
