public void FindPath () {
 ...
 public void FindPath () {
    ManagementObjectSearcher discover = new ManagementObjectSearcher("SELECT * FROM Win32_DiskDrive");
    foreach (ManagementObject obj in discover.Get()) {
        if (obj["PNPDeviceID"].ToString().Contains("USBSTOR")) {
            if (! USBobjects.Where(x => x.Equals(obj["PNPDeviceID"].ToString())).Any()){
                USBobjects.Add(obj["PNPDeviceID"].ToString());
            }
        }
    }
