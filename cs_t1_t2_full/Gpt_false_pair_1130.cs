...
static int Main (string [] args)
{
    ...
    } Console.WriteLine ();
    {
        ...
        stopwatch.Start ();
        for (int i = 0; i < LoopCount; i ++) {
            maxValue = num1 ^ ((num1 ^ num2) & -(num1 < num2));
        }
        stopwatch.Stop ();
        Console.WriteLine ("Bitwise: " + stopwatch.Elapsed.TotalSeconds + " seconds");
        Console.WriteLine ("Relative: " + (stopwatch.Elapsed.TotalSeconds - controlTotalSeconds) + " seconds");
    } Console.WriteLine ();
    {
        var stopwatch = new Stopwatch ();
        stopwatch.Start ();
        for (int i = 0; i < LoopCount; i ++) {
            var bigger = num1 - (num1 - num2);
            maxValue = bigger;
        }
        stopwatch.Stop ();
        Console.WriteLine ("Subtraction: " + stopwatch.Elapsed.TotalSeconds + " seconds");
        Console.WriteLine ("Relative: " + (stopwatch.Elapsed.TotalSeconds - controlTotalSeconds) + " seconds");
    } Console.ReadLine ();
    return maxValue;
}
