...
void btnApplica_Click_Scelta_Campi_Etichetta_variant3 (object sender, EventArgs e) {
    Debug.Print ("btnApplica_Click_Scelta_Campi_Etichetta_variant3 START");
    string id;
    if (sender.GetType () == typeof (ImageButton)) {
        ImageButton button = (ImageButton) sender;
        id = button.ID;
    ...
    string address = web.Url;
    string link = address + "/ARXEIA WEBPART/Carica documento.aspx?mode=scelta_campi_facoltativi_etichetta&obj=" + obj;
    SPUtility.Redirect (link, SPRedirectFlags.Default, Context);
}
