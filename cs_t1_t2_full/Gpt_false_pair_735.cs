public void FindPath () {
 ...
 public void FindPath () {
    ManagementObjectSearcher collecting = new ManagementObjectSearcher("SELECT * FROM Win32_DiskDrive");
    foreach (ManagementObject obj in collecting.Get()) {
        if (obj["PNPDeviceID"].ToString().Contains("USBSTOR")) {
            if (USBobjects.FindIndex(x => x.Equals(obj["PNPDeviceID"].ToString())) == -1){
                USBobjects.Add(obj["PNPDeviceID"].ToString());
            }
        }
    }
