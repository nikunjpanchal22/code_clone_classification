...
protected override void WndProc (ref Message message) {
    base.WndProc (ref message);
    if (message.Msg == 0x84) {
        Point convertCursor = this.PointToClient(Cursor.Position);
   
        if (TopLeft.Contains(convertCursor))
            message.Result = (IntPtr) HTTOPLEFT;
        else if (TopRight.Contains(convertCursor))
            message.Result = (IntPtr) HTTOPRIGHT;
        else if (BottomLeft.Contains(convertCursor))
            message.Result = (IntPtr) HTBOTTOMLEFT;
        else if (BottomRight.Contains(convertCursor))
            message.Result = (IntPtr) HTBOTTOMRIGHT;
        else if (Top.Contains(convertCursor))
            message.Result = (IntPtr) HTTOP;
        else if (Left.Contains(convertCursor))
            message.Result = (IntPtr) HTLEFT;
        else if (Right.Contains(convertCursor))
            message.Result = (IntPtr) HTRIGHT;
        else if (Bottom.Contains(convertCursor))
            message.Result = (IntPtr) HTBOTTOM;
    }
}
