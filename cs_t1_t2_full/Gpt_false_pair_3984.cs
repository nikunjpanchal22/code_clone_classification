...
static void Run (string [] compile) {
    Console.Clear ();
    Console.ForegroundColor = ConsoleColor.Gray;
    Dictionary < string, string [] > sprites = new Dictionary < string, string [] > ();
    sprites.Add ("ahri", new string [] {"Academy", "Challenger", "Dynasty", "Foxfire", "Midnight", "Popstar"});
    sprites.Add ("leesin", new string [] {"Traditional", "Acolyte", "Dragon Fist", "Musy Thai", "Pool Party", "SKT T1", "Knockout"});
    Console.WriteLine ("Conor's Random League of Legends Sprite Selector v0.1\r\n\r\n");
    Console.WriteLine ("What champion would you like to select a sprite for? \r\nPress enter for a random champion...    ");
    var champion = Console.ReadLine ();
    var rand = new Random ();
    if (champion.Equals (string.Empty)) {
        var tmpList = Enumerable.ToList (sprites.Keys);
        champion = tmpList [rand.Next (tmpList.Count)];
    } else {
        champion = champion.Trim ().ToLower ();
    }
    Console.Write (string.Format ("Champion {0} Selected \r\n", champion));
    if (sprites.ContainsKey (champion)) {
        Console.WriteLine (string.Format ("Your random sprite for {0} is: {1}\r\n", champion, sprites [champion] [rand.Next (sprites [champion].Length)]));
    } else {
        Console.Clear ();
        Run (compile);
    }
