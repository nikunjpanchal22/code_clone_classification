...
public static void Main (string [] args) {
    int ? second = null, third = null;
    int second_i = 0, third_i = 0;
    while (! second.HasValue) {
        Console.WriteLine ("enter second number");
        if (int.TryParse (Console.ReadLine (), out second_i)) {
            second = second_i;
    ...
    while (! third.HasValue) {
        Console.WriteLine ("enter third number");
        if (int.TryParse (Console.ReadLine (), out third_i)) {
            third = first_i;
        }
    }
    int product = computeProd3 (second_i, third_i);
    Console.WriteLine ("Their product is:\t" + product);
    Console.Read ();
}
