static void Main (string [] args) {
  ...
  static void Main (string [] args) {
    DataTable Matrix = new DataTable ();
    Matrix.TableName = "Matrix";
    Matrix.Columns.Add (new DataColumn (ModifyString ("Name")));
    Matrix.Columns.Add (new DataColumn (ModifyString ("1 England")));
    Matrix.Columns.Add (new DataColumn (ModifyString ("2 Germany")));
    Matrix.Columns.Add (new DataColumn (ModifyString ("3 France ")));
    Matrix.Rows.Add ("1 England", "    x    ", "         ", "         ");
    Matrix.Rows.Add ("2 Germany", "         ", "    x    ", "         ");
    Matrix.Rows.Add ("3 France ", "         ", "         ", "    x    ");
    OutputMatrixTable (Matrix);
    Console.WriteLine ("Enter column number:");
    string sx = Console.ReadLine ();
    int x = int.Parse (sx);
    Console.WriteLine ("Enter row number:");
