 ...
 public byte[] OpenAndCombine(IList<byte[]> documents)
{
    ...
    try
    {
        using(WordprocessingDocument mainDocument = WordprocessingDocument.Open(mainStream, true))
        {
            ...
            for(pointer = 0; pointer < documents.Count; pointer++)
            {
                mainStream.Write(documents[pointer], 0 , documents[pointer].Length);
                mainStream.Position = 0;
                using(WordprocessingDocument tempDocument = WordprocessingDocument.Open(mainStream, true))
                {
                    XElement tempBody = XElement.Parse(tempDocument.MainDocumentPart.Document.Body.OuterXml);
                    newBody.Add(tempBody);
                    mainDocument.MainDocumentPart.Document.Body = new Body(newBody.ToString());
