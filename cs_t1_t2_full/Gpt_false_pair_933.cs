public static void d (Exception e) {
    ...
    try {
        ...
        string methodName = site == null ? "" : site.Name;
        methodName = ExtractBracketed (methodName);
        StackTrace stkTrace = new System.Diagnostics.StackTrace (e, false);
        for (int i = 0; i < 3; i ++) {
            var frame = stkTrace.GetFrame (i);
            int lineNum = frame.GetFileLineNumber ();
            int colNum = frame.GetFileColumnNumber ();
            string className = ExtractBracketed (frame.GetMethod ().ReflectedType.FullName);
            Console.WriteLine (ThreadAndDateInfo + "Exception: " + className + "." + methodName + ", Ln " + lineNum + " Col " + colNum + ": " + e.Message);
            if (lineNum + colNum > 0)
                break;
        }
    }
    catch (Exception ee) {
        Trace.WriteLine ("Tracing exception in d(Exception e)" + ee.Message);
    }
}
