...
public void MapDataReaderListObjects<T>(IDataReader dr, List<T> list) where T : new()
{
    Type businessEntityType = typeof(T);
    List<T> entities = new List<T>();
    T myObject = new T();
    LoadProperties(myObject, businessEntityType);
    ...
    while (dr.Read())
    {
        ...
        for (int index = 0; index < dr.FieldCount; index++)
        {
            for (int counter = 0; counter < sourceProperties.Count; counter++)
            {
                if (sourceProperties[counter].Name.ToUpper() == dr.GetName(index).ToUpper())
                {
                    string propertyType = sourceProperties[counter].PropertyType.ToString();
                    PropertyInfo info = sourceProperties[counter] as PropertyInfo;
                    if (info != null && info.CanWrite)
                    {
                        info.SetValue(newObject, dr.GetValue(index), null);
                    }
