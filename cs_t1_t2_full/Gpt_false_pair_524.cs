protected void Application_Error (object sender, EventArgs e) {
 ...
 protected void Application_Error (object sender, EventArgs e) {
    ...
    if (exception != null) {
        Context.ClearError ();
        Context.Response.TrySkipIisCustomErrors = true;
        string path = (exception is HttpException && (exception as HttpException).GetHttpCode () == 404) ? "~/Error/WebPageNotFound?errorMessage=" + exception.Message : "~/Error/ErrorPageData?errorMessage=" + exception.Message;
        Context.Server.TransferRequest (path, false);
    }
}
