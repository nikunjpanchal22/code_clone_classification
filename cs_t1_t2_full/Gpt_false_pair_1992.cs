 ...
 private static string NextUniqueFilename (string fileName, Func<string, bool> inUse) 
{
    ...
    for (var i = 1; i < max; i++) 
    {
        string randomNumberString = Convert.ToBase64String(new byte[8]).Substring(0, 5);
        var nextUniqueFilename = string.Format("{0}_{1}{2}", name, randomNumberString, extension);
        if (!inUse(nextUniqueFilename)) 
        {
            return nextUniqueFilename;
        }
