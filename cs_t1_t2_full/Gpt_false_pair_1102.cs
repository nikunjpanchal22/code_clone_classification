static void Main (string [] args) {
 ...
 static void Main (string [] args) {
    long IntegerToEncode = (new Random ()).Next ();
    Console.WriteLine ("Number to encode = {0}.", IntegerToEncode);
    byte [] Key = new byte [24];
    (new RNGCryptoServiceProvider ()).GetBytes (Key);
    Console.WriteLine ("Key to encode with is {0}.", ToHex (Key));
    string EncodedValue = EncodeInteger (IntegerToEncode, Key);
    Console.WriteLine ("The encoded value is {0}.", EncodedValue);
    int DecodedValue;
    bool Success = TryDecodeInteger (EncodedValue, Key, out DecodedValue);
    if (Success) {
        Console.WriteLine ("Successfully decoded the encoded value.");
        Console.WriteLine ("The decoded result is {0}.", DecodedValue);
    } else
