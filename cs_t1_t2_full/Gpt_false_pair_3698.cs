...
protected void MakeButtons () {
    ...
    for (int row = 0; row < nr; row ++) {
        for (int column = 0; column < nc; column ++) {
            Button btnNew = new Button ();
            btnNew.Name = "btn_" + row + "_" + column;
            btnNew.Height = btnHeight - 10;
            btnNew.Width = btnWidth - 10;
            btnNew.Font = new Font ("Calibri", 20);
            btnNew.Image = Properties.Resources.background;
            btnNew.Visible = true;
            btnNew.Location = new Point (20 + (column * btnNew.Width), 20 + (row * btnNew.Height));
            btnNew.Click += new EventHandler (WhichClicked);
            panel1.Controls.Add (btnNew);
        }
    }
}
