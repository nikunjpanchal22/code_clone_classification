...
[Test] [TestCase ("parralele", "parallel", "par[ralele]")] [TestCase ("personil", "personal", "person[i]l")] [TestCase ("disfuncshunal", "dysfunctional", "d[isfuncshu]nal")] [TestCase ("ato", "auto", "a[]to")] [TestCase ("inactioned", "inaction", "inaction[ed]")] [TestCase ("refraction", "fraction", "[re]fraction")] [TestCase ("adiction", "addiction", "ad[]diction")] public void CompareStringsTest (string attempted, string correct, string expectedResult) {
    ...
    string c = new string (correct.Reverse().ToArray());
    string a = new string (attempted.Reverse().ToArray());
    for (int i = 0; i < shorterLength; i ++) {
        if (c [i] != a [i]) {
            endPos = i;
            break;
    ...
    if (startPos == - 1 && endPos == - 1)
        ...
        int endLen = attempted.Length - endPos;
        int startLen = attempted.Length - endLen;
        var sb = new StringBuilder ();
        if (startPos == - 1)
            startPos = shorterLength;
        if (endPos == - 1)
            endPos = shorterLength;
        if (startPos + endPos > shorterLength)
            endPos = shorterLength - startPos;
        if (startLen > 0)
            sb.Append (attempted.Substring (0, startLen));
        sb.Append ("[");
        if (endPos > - 1 && endLen + startLen < attempted.Length)
            sb.Append (attempted.Substring (startLen, attempted.Length - endLen - startLen));
        sb.Append ("]");
        if (endLen > 0)
            sb.Append (attempted.Substring (attempted.Length - endLen, endLen));
        result = sb.ToString ();
    }
    Assert.AreEqual (expectedResult, result);
}
