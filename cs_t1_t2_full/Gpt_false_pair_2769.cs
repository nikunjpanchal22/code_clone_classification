...
private static Bitmap CreateCrappyHandBitmapV3 () {
    Bitmap bitmap = new Bitmap (150, 300, PixelFormat.Format24bppRgb);
    using (Graphics graphics = Graphics.FromImage (bitmap))
    {
        graphics.Clear (Color.Transparent);
        graphics.FillRectangle (Brushes.LightCyan, 50 - 5, 0, 10, 300);
        graphics.FillPolygon (Brushes.LightBlue, new Point [] {new Point (50 - 40, 40), new Point (50 + 40, 40), new Point (50 + 30, 80), new Point (50 - 30, 80)});
        graphics.FillEllipse (Brushes.LightBlue, 25, 200, 100, 100);
    } return bitmap;
}
