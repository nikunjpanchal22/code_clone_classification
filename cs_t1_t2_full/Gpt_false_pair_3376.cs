...
public static string ReplaceHashtagsWithInt (string input, int integer) {
    ...
    for (Match match = regex.Match (input); match.Success; match = match.NextMatch ()) {
        ...
        if (num.Length <= match.Length)
            for (int i = 0; i < match.Length; i ++) {
                if (i < match.Length - num.Length)
                    output [match.Index + i + diff] = '0';
                else
                    output [match.Index + i + diff] = num [i - match.Length + num.Length];
            }
        else {
            output.Remove (match.Index + diff, match.Length);
            output.Insert (match.Index + diff, num);
            diff += num.Length - match.Length;
        }
    }
    return output.ToString ();
}
