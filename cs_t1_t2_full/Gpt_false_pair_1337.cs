static void Main (string [] args) {
  ...
  static void Main (string [] args) {
    ...
    using (WindsorDbConnection cn = new WindsorDbConnection (csv.ConnectionString))
    {
        cn.Open ();
        using (WindsorDbCommand cmd = cn.CreateCommand ())
        {
            cmd.CommandText = "SELECT * FROM [Stats.csv]";
            cmd.CommandType = CommandType.Text;
            using (WindsorDbDataReader reader = cmd.ExecuteReader (CommandBehavior.CloseConnection))
            {
                int fieldSport = reader.GetOrdinal ("sport");
                int fieldDate = reader.GetOrdinal ("date");
                int fieldTeamOne = reader.GetOrdinal ("teamone");
        ...
        System.Windows.MessageBox.Show(stat.Sport);
    }
}
