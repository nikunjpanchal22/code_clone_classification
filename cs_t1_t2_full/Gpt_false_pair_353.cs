...
public static SqlBytes BinaryCompress (SqlBytes input)
{
    ...
    using (MemoryStream result = new MemoryStream ())
    {
        using (ZstandardStream zstandardStream = new ZstandardStream (result, CompressionMode.Compress, true))
        {
            zstandardStream.Write (input.Buffer, 0, input.Buffer.Length);
