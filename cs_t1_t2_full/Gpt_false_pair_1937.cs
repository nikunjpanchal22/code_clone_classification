protected void btnExport_Click (object sender, EventArgs e) {
  ...
  protected void btnExport_Click (object sender, EventArgs e) {
    try {
        ...
        foreach (GridViewRow row in GridView1.Rows) {
            CheckBox chkNameRow = (CheckBox) row.FindControl ("chkName");
            DropDownList ddlCountryRow = (DropDownList) row.FindControl ("ddlCountry");
            DataRow drow = dt.NewRow ();
            for (int i = 0; i < GridView1.Columns.Count; i ++) {
                drow [i] = row.Cells [i].Text;
            }
            drow ["Name"] = chkNameRow.Checked.ToString();
            drow ["Country"] = ddlCountryRow.SelectedValue;
            dt.Rows.Add (drow);
        }
        using (XLWorkbook wb = new XLWorkbook ())
        {
