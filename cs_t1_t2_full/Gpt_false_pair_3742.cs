...
void NotAccessibleError (FileSystemWatcher source, ErrorEventArgs e) {
    ...
    int iTimeOut = 30000;
    int i = 0;
    while ((! File.Exists (source.Path) || source.EnableRaisingEvents == false) && i < iMaxAttempts) {
        i += 1;
        try {
            source.EnableRaisingEvents = false;
            if (! File.Exists (source.Path)) {
                MyEventLog.WriteEntry ("File Inaccessible " + source.Path + " at " + DateTime.Now.ToString ("HH:mm:ss"));
                System.Threading.Thread.Sleep (iTimeOut);
            } else {
                source.Dispose ();
                source = null;
            ...
            MyEventLog.WriteEntry ("Error trying Restart Service " + error.Message + " at " + DateTime.Now.ToString ("HH:mm:ss"));
            source.EnableRaisingEvents = false;
            System.Threading.Thread.Sleep (iTimeOut);
        }
    }
