...
static void Main (string [] args) {
    var l = new List < List < double > > () {new List < Double > () {0, 16.0000, 15.0000, 0, 2.7217, 3.7217}, new List < Double > () {0, 0, 15.0000, 15.0000, 5.6904, 5.6904}};
    int k = 1;
    var resultado = from sublista in l
        select new {minimo = sublista.Min (), maximo = sublista.Max (), indice = k ++};
    foreach (var r in resultado)
        Console.WriteLine (String.Format ("Index: {0} Min: {1} Max: {2}", r.indice, r.minimo, r.maximo));
    Console.ReadKey ();
}
