...
static public bool AddUserToGroup (string user, UserGroup group) {
    ...
    var sid = new IntPtr ();
    switch (group) {
        case UserGroup.NetworkConfigurationOperators :
            ConvertStringSidToSid ("S-1-5-32-556", out sid);
            break;
        case UserGroup.RemoteDesktopUsers :
            ConvertStringSidToSid ("S-1-5-32-555", out sid);
            break;
    }
    SID_NAME_USE sidType;
    if (! LookupAccountSid (null, sid, name, ref nameSize, refDomainName, ref refDomainNameSize, out sidType))
