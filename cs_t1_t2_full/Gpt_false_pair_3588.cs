...
public string Insert ()
{
    ...
    try 
    {
        ...
        using (SqlConnection Conn = new SqlConnection (conStr))
        {
            using (SqlCommand cmd = new SqlCommand (querySQL, Conn))
            {
             	cmd.Parameters.Add ("@name", SqlDbType.VarChar).Value = inputName;
                cmd.Parameters.Add ("@email", SqlDbType.VarChar).Value = inputEmail;
                cmd.Parameters.Add ("@password", SqlDbType.VarChar).Value = inputPassword;
                queryResult = cmd.ExecuteNonQuery ();
            }} return queryResult + "Record Inserted successfully!";
    }
    catch (SqlException ex)
    {
        if (ex.Number == 2627) 
        {
            return "Entered Record already exist";
        }
