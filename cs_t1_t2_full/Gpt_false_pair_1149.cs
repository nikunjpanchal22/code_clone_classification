[HttpPost] [ValidateAntiForgeryToken] public ActionResult Create (Movie movie, HttpPostedFile fileUpload) {
 ...
 [HttpPost] [ValidateAntiForgeryToken] public ActionResult Create (Movie movie, IFormFileCollection fileCollection) {
    if (ModelState.IsValid) {
        db.MovieContext.Add (movie);
        db.SaveChanges ();
        foreach(var postedFile in fileCollection) {
            postedFile.CopyTo (Path.Combine ("~/UploadedFiles", pelicula.Id));
        }
        return RedirectToAction ("Index");
    }
    var content = "";
