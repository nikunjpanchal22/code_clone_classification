private void Interval (object sender, EventArgs e) {
 ...
 private void Interval (object sender, EventArgs e) {
    ...
    if (cbPause.Checked) {
        ...
        randomLine = random.Next (lbMessage.Items.Count);
        // Set the timer setting to enabled
        tmrSpace.Enabled = true;
        
        // Update the whenStart boolean
        if (whenStart) {
            tickCount ++;
        // If the boolean has not been updated
        } else {
            whenStart = true;
        }
    } else {
        ...
        // Otherwise
            ...
            // Check if the tickCount is below the number of items in the list
            if (tickCount < lbMessage.Items.Count) {
    ...
    if (tickCount == lbMessage.Items.Count)
        ...
        tickCount = 0;
    // Invoke the SetInterval function
    SetInterval ();
}
