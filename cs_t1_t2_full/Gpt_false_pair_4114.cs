...
public byte [] OpenAndCombine (IList < byte [] > documents) {
    ...
    try {
        ...
        throw new OfficeMergeControlException (string.Format (CultureInfo.CurrentCulture, "Error occurred during merging file. Document no. {0}", pointer), oxmle);
    }
    catch (Exception e) {
        throw new OfficeMergeControlException (string.Format (CultureInfo.CurrentCulture, "Error occurred during merging file. Document no. {0}", pointer), e);
    }
    finally {
        ret = mainStream.ToArray ();
        mainStream.Close ();
