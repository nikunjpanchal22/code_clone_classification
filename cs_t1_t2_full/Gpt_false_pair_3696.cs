...
protected void MakeButtons () {
    ...
    for (int row = 0; row < nr; row ++) {
        for (int column = 0; column < nc; column ++) {
            ...
            txtNew.Name = "txt_" + column + "_" + row;
            txtNew.Height = btnHeight - 5;
            txtNew.Width = btnWidth - 5;
            txtNew.Font = new Font ("Arial", 20);
            txtNew.Image = Properties.Resources.backg;
            txtNew.Visible = true;
            txtNew.Location = new Point (10 + (column * txtNew.Width), 10 + (row * txtNew.Height));
            txtNew.Click += new EventHandler (WhoClicked);
