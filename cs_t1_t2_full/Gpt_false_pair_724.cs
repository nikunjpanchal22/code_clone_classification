public override void WriteJson (JsonWriter writer, object value, JsonSerializer serializer) {
 ...
 public override void WriteJson (JsonWriter writer, object value, JsonSerializer serializer) {
    ...
    if (valueType.IsArray) {
        ...
        JProperty typeHintProperty = new JProperty("type", value.GetType ().Name + "_data"); 
        var jObj = new JObject ();
        if (typeHintProperty != null)
            jo.Add (typeHintProperty);
        foreach (PropertyInfo property in valueType.GetProperties (BindingFlags.Public | BindingFlags.Instance)) {
            if (property.CanRead) {
                object propertyValue = property.GetValue (value);
                if (propertyValue != null)
                    jObj.Add ( valueType.Name + "_" + property.Name, JToken.FromObject (propertyValue, serializer));
            }
        }
        jObj.WriteTo (writer);
    }
