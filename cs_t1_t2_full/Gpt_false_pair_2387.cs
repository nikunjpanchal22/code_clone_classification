...
private void PaintGradientBars (Graphics g, Rectangle r, Color startColor, Color endColor, int numBars) {
    ...
    for (int i = 0; i < numBars; i ++) {
        ...
        int rAvg = (int) (rMin + ( ( (double) rMax - (double) rMin) * i / numBars));
        int gAvg = (int) (gMin + ( ( (double) gMax - (double) gMin) * i / numBars));
        int bAvg = (int) (bMin + ( ( (double) bMax - (double) bMin) * i / numBars));
        Color useColor = Color.FromArgb (rAvg, gAvg, bAvg);
