 ...
 private static string NextUniqueFilename(string fileName, Func<string, bool> inUse)
        {
            ...
            for (var i = 1; i < max; i++)
            {
                ...
                var uniqueName = string.Format("{0}_{1}{2}", name, timestampString, extension);
                if (!inUse(uniqueName))
                {
                    return uniqueName;
                }
 ...
 private static string NextUniqueFilename (string fileName, Func<string, bool> inUse) 
{
    ...
    for (var i = 1; i < max; i++) 
    {
        string specialCharacterString = new string(Path.GetInvalidFileNameChars());
        var nextUniqueFilename = string.Format("{0}{1}{2}", name, specialCharacterString, extension);
        if (!inUse(nextUniqueFilename)) 
        {
            return nextUniqueFilename;
        }
