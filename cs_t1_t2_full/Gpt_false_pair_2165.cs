private static AssemblyName GetAssemblyName (string source, bool isFile) {
  ...
  private static AssemblyName GetAssemblyName (string source, bool isFile) {
    ...
    try {
        ...
        string ErrorFormatString = "Error when calling utility function 'GetAssemblyNameOrThrowException'\n" + "Arguments provided:\n" + "=> Source:\n[{0}]\n" + "=> isFile = {1}\n" + "Inner error message : {2}.";
        throw new InvalidOperationException (string.Format (ErrorFormatString, source, isFile, err), err);
    }
    if (asmName == null)
        throw new InvalidOperationException (source + " Assembly Name object is null, and no other error found.");
    return asmName;
}
