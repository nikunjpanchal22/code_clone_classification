...
private void SpCaller_DoWork (object sender, DoWorkEventArgs e) {
    var self = (BackgroundWorker) sender;
    var cb = new SqlConnectionStringBuilder {DataSource = ".", InitialCatalog = "Sandbox", IntegratedSecurity = false};
    using (var cn = new SqlConnection (cb.ToString ()))
    {
        cn.FireInfoMessageEventOnUserErrors = false;
        cn.Open ();
        cn.InfoMessage += (o, args) => self.ReportProgress (100, args.Message);
        using (var cmd = cn.CreateCommand ())
        {
            cmd.CommandText = "usp_ShortProcess";
