public bool doFindSum (ref int [] nums, int index, int current, int target) {
  ...
  public bool doFindSum (ref int [] nums, int index, int current, int target) {
    ...
    bool multiply = doFindSum (ref nums, index + 1, current * nums [index + 1], target);
    bool modulus = doFindSum (ref nums, index + 1, current % nums [index + 1], target);
    return multiply || modulus;
}
