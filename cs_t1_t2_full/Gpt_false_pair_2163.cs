private static AssemblyName GetAssemblyName (string source, bool isFile) {
  ...
  private static AssemblyName GetAssemblyName (string source, bool isFile) {
    ...
    try {
        ...
        string ErrorFormatString = "Invalid call to util function 'GetAssemblyNameOrThrowException' \n" + "Parameters supplied were:\n" + "=> Source:\n[{0}]\n" + "=> isFile = {1}\n" + "See inner exception for error info.";
        throw new InvalidOperationException (string.Format (ErrorFormatString, source, isFile), err);
    }
    if (asmName == null)
        throw new InvalidOperationException (source + " Assembly Name object is null, no other error was found.");
    return asmName;
}
