...
protected void btnSubmit_Click3 (object sender, EventArgs e) {
    ...
    int cnt;
    using (SqlConnection conn = new SqlConnection ("Server; Database; Integrated security = true"))
    using (SqlCommand cmd = new SqlCommand ("Insert into T_TADA_tempform(EMPID,DIVID,DesigID,RelaseDate) values(@EMPID,@DIVID,@DesigID,@RelaseDate)", conn))
    {
        cmd.Parameters.AddWithValue ("@EMPID", ddlname.SelectedValue);
        cmd.Parameters.AddWithValue ("@DIVID", lbldesig.Text);
        cmd.Parameters.AddWithValue ("@DesigID", lbldiv.Text);
        cmd.Parameters.AddWithValue ("@RelaseDate",RelaseDate);
        conn.Open ();
        cnt = cmd.ExecuteNonQuery ();
    } if (cnt == 1) {
        Response.Redirect ("form.aspx");
