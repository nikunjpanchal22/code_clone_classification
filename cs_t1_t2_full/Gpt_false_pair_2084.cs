public static IEnumerable < T > MergePreserveOrder4 < T, TOrder > (this IEnumerable < IEnumerable < T > > aa, Func < T, TOrder > orderFunc) where TOrder : IComparable < TOrder > {
 ...
 public static IEnumerable < T > MergePreserveOrder4 < T, TOrder > (this IEnumerable < IEnumerable < T > > aa, Func < T, TOrder > orderFunc) where TOrder : IComparable < TOrder > {
   var items = aa.Select (xx => xx.GetEnumerator ()).Where (ee => ee.MoveNext ()).Select (ee => Tuple.Create (orderFunc (ee.Current), ee)).OrderBy (ee => ee.Item1).ToArray ();
   while (items.Length > 0) {
       ...
       if (next.Item2.MoveNext ()) {
           ...
           for (; ii < items.Length; ++ ii) {
               if (value.CompareTo (items [ii].Item1) <= 0) {
                   items = items.Take(ii).Append(Tuple.Create (value, next.Item2)).Concat(items.Skip(ii)).ToArray();
                   break;
               }
           }
           if (ii == items.Length)
               Array.Resize(ref items, items.Length + 1); 
               items[items.Length - 1] = Tuple.Create (value, next.Item2); 
       } else
           next.Item2.Dispose ();
   }   
}
