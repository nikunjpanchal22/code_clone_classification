private void myBtn_Click (object sender, EventArgs e) {
 ...
 private void myBtn_Click (object sender, EventArgs e) {
    if (dataGridView1.SelectedRows.Count > 0) {
        var selectedIndex = myDGV.SelectedRows [0].Index;
        var row = myDGV.Rows [selectedIndex];
        var formLoc = this.Location;
        var gridLoc = myDGV.Location;
        var rowStartLoc = myDGV.GetRowDisplayRectangle (selectedIndex, false).Location;
        newForm form = new newForm ();
        form.StartPosition = FormStartPosition.Manual;
        form.Location = GetPopupPosition (new Point [] {formLoc, gridLoc, rowStartLoc}, row.Height);
        form.Show (this);
    }
}
