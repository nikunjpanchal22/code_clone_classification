...
protected override void WndProc (ref Message message) {
    base.WndProc (ref message);
    if (message.Msg == 0x84) {
        Point posCurrent = this.PointToClient (Cursor.Position);
          
        if (TopLeft.Contains(posCurrent))
            message.Result = (IntPtr) HTTOPLEFT;
        else if (TopRight.Contains(posCurrent))
            message.Result = (IntPtr) HTTOPRIGHT;
        else if (BottomLeft.Contains(posCurrent))
            message.Result = (IntPtr) HTBOTTOMLEFT;
        else if (BottomRight.Contains(posCurrent))
            message.Result = (IntPtr) HTBOTTOMRIGHT;
        else if (Top.Contains(posCurrent))
            message.Result = (IntPtr) HTTOP;
        else if (Left.Contains(posCurrent))
            message.Result = (IntPtr) HTLEFT;
        else if (Right.Contains(posCurrent))
            message.Result = (IntPtr) HTRIGHT;
        else if (Bottom.Contains(posCurrent))
            message.Result = (IntPtr) HTBOTTOM;
    }
}
