protected void btnExport_Click (object sender, EventArgs e) {
  ...
  protected void btnExport_Click (object sender, EventArgs e) {
    try {
        ...
        foreach (GridViewRow row in GridView1.Rows) {
            DropDownList ddlNameRow = (DropDownList) row.FindControl ("ddlName");
            CheckBox chkCountryRow = (CheckBox) row.FindControl ("chkCountry");
            DataRow drow = dt.NewRow ();
            for (int i = 0; i < GridView1.Columns.Count; i ++) {
                drow [i] = row.Cells [i].Text;
            }
            drow ["Name"] = ddlNameRow.SelectedItem.Text;
            drow ["Country"] = chkCountryRow.Checked.ToString();
            dt.Rows.Add (drow);
        }
