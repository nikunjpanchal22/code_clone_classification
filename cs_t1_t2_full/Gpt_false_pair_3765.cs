...
public virtual bool IsEqual (EntityBase other) {
    if (other == null) {
        return false;
    }
    if (ReferenceEquals (other, this)) {
    ...
    var otherClass = NHibernateProxyHelper.GetClassWithoutInitializingProxy (other);
    var thisClass = NHibernateProxyHelper.GetClassWithoutInitializingProxy (this);
    if (! otherClass.Equals (thisClass)) {
        return false;
    }
    bool isOtherTransient = Equals (other.Id, 0);
