...
private void OnUnhandledApplicationException(object sender, EventArgs e)
{
    ...
    if (currentException != null)
    {
        message.AppendFormat("<tr><td class=\"header2\"><![CDATA[TYPE</td></tr><tr><td>{0}</td></tr><tr><td class=\"header2\"><![CDATA[MESSAGE</td></tr><tr><td>{1}</td></tr><tr><td class=\"header2\"><![CDATA[STACK TRACE</td></tr><tr><td>{2}</td></tr>", currentException.GetType().FullName, currentException.Message, currentException.StackTrace);
        server.ClearError();
    }
    else
    {
        message.Append("<tr><td class=\"header2\"><![CDATA[No unhandled exceptions found]</td></tr>");
    }
    HttpContext.Current.Response.Write(message.ToString());
    server.ClearError();
}
