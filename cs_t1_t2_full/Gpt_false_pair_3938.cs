...
public string FormatNumDigits (double number, int x) {
    string asString = (number >= 0 ? "" : "-") + number.ToString ("F50", System.Globalization.CultureInfo.InvariantCulture);
    if (asString.Contains ('.')) {
        if (asString.Length > x + 4) {
            return asString.Substring (0, x + 4);
        } else {
            return asString.Insert (asString.Length, new String ('0', x + 4 - asString.Length));
        }
    } else {
        if (asString.Length > x + 3) {
            return asString.Substring (0, x + 3);
        } else {
            return asString.Insert (asString.Length, new String ('0', x + 3 - asString.Length));
        }
    }
}
