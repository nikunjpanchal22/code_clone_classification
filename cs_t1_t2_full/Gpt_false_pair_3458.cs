...
[PermissionSetAttribute (SecurityAction.Demand, Name = "FullTrust")] public static WindowsImpersonationContext doImpersonation (string svcUserName, string domainName, string password) {
    IntPtrTokenHandle = IntPtr.Zero;
    IntPtrDupeTokenHandle = IntPtr.Zero;
    bool returnValue = LogonUserEx (svcUserName, domainName, password, LOGON32_LOGON_INTERACTIVE, LOGON32_PROVIDER_WINNT50, ref tokenHandle);
    if (returnValue == false) {
        int ret = Marshal.GetLastWin32Error ();
        if (ret != NO_ERROR)
            throw new Exception ("LogonUserEx failed with error code : " + GetError (ret));
    }
    bool retVal = DuplicateTokenEx (tokenHandle, SecurityImpersonation, 0, ref dupeTokenHandle);
    if (retVal == false) {
        CloseHandle (tokenHandle);
        throw new Exception ("Exception thrown in trying to duplicate token.");
    } else {
