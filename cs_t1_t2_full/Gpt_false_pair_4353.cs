...
static void Main (string [] args) {
    ...
    (new RNGCryptoServiceProvider ()).GetBytes (EncryptionKey);
    Console.WriteLine ("Key to encode with is {0}.", ToHex (EncryptionKey));
    string EncodedValue = Encode (NumberToEncode, EncryptionKey);
    Console.WriteLine ("The encoded value is {0}.", EncodedValue);
    long DecodedValue;
    bool Result = TryDecode (EncodedValue, EncryptionKey, out DecodedValue);
    if (Result) {
        Console.WriteLine ("Successfully decoded the encoded value.");
        Console.WriteLine ("The decoded result is {0}.", DecodedValue);
    } else
