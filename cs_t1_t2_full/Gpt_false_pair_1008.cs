private static string ReplaceBBCode (string inStr) {
 ...
 private static string ReplaceBBCode(string inStr) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            { 
                addBR = false;
                if (i + 2 < length && inStr[i] == '[' && (inStr[i + 1] == 'b' || inStr[i + 1] == 'i' || inStr[i + 1] == 'u') && inStr[i + 2] == ']')
                { 
                    outStr.Append("<").Append(inStr[i + 1]).Append("><![CDATA["); 
                    i += 2;
                }
                else if (i + 3 < length && inStr[i] == '[' && inStr[i + 1] == '/' && (inStr[i + 2] == 'b' || inStr[i + 2] == 'i' || inStr[i + 2] == 'u') && inStr[i + 3] == ']')
                {
                    outStr.Append("</").Append(inStr[i + 2]).Append("><![CDATA["); 
                    i += 3;
                }
