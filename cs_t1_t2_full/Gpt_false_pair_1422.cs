private void SetTableDescriptions (Type tableType) {
  ...
  private void SetTableDescriptions (Type tableType) {
    ...
    foreach (var attribute in tableType.GetTypeInfo().CustomAttributes) {
        if (attribute.AttributeType == typeof(DescriptionAttribute)) {
            SetTableDescription(tableName, (string)attribute.ConstructorArguments[0].Value);
        }
    }
    ...
    foreach (var prop in tableType.GetProperties (System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)) {
        if (prop.PropertyType.IsClass && prop.PropertyType != typeof (string))
            continue;
        var descriptionAttributes = prop.GetCustomAttributes (typeof (DescriptionAttribute), false);
        if (descriptionAttributes != null && descriptionAttributes.Length > 0) {
            SetColumnDescription (tableName, prop.Name, ((DescriptionAttribute) descriptionAttributes [0]).Description);
        }
    }
}
