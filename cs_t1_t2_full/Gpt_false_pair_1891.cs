private void txt_miktar_TextChanged (object sender, TextChangedEventArgs e) {
    ...
    try {
        int total = Convert.ToInt32 ((sender as TextBox).Text);
    }
    catch (Exception) {
        string word = "";
        word = (sender as TextBox).Text;
        word = word.Substring (0, word.Length - 1);
        (sender as TextBox).Text = word;
        (sender as TextBox).Select (word.Length, word.Length);
    }
}
