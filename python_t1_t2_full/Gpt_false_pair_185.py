def signal_handler(helloProcess, counterProcess, signum, frame) :
 ...
 def signal_handler(helloProcess, counterProcess, signum, frame) :
	...
	if signum == 17 :
		...
		signal(SIGCHLD, SIG_IGN)
		if helloProcess.is_alive() :
			print("Sending SIGCONT to helloProcess")
			os.kill(helloProcess.pid, signal.SIGCONT)
		if counterProcess.is_alive() :
			print("Sending SIGCONT to counterProcess")
			os.kill(counterProcess.pid, signal.SIGCONT)
		sys.exit(0)
