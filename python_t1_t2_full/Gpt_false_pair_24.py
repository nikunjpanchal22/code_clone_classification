def __init__(self, parent = None) :
  ...
  def __init__(self, parent = None) :
		...
		self.model = QtGui.QStandardItemModel(self.list_view)
		self.model.itemChanged.connect(self.setItems)
		file_opened = QtCore.QFile(loo)
		if file_opened.open(QtCore.QFile.ReadOnly | QtCore.QFile.Text) :
			reader= QtCore.QTextStream(file_opened)
			while not reader.atEnd() :
				line = bytearray(reader.readLine()).decode().strip()
				item = QtGui.QStandardItem(line)
				item.setCheckable(True)
				item.setCheckState(QtCore.Qt.Unchecked)
				self.model.appendRow(item)
