...
def __init__(self, pos, checkpoints) :
	super().__init__()
	self.surf = pg.Surface((60, 60), pg.SRCALPHA)
	pg.draw.polygon(self.surf, (0, 100, 240), [(30, 0), (60, 60), (0, 60)])
	self.rect = self.surf.get_rect(center = pos)
	self.mask = pg.mask.from_surface(self.surf)
	self.checkpoints = itertools.cycle(checkpoints)
	self.cur_checkpoint = next(self.checkpoints)
	self.start_point = self.cur_checkpoint
	self.cur_checkpoint.image = self.cur_checkpoint.image_active
	self.rounds = -1
