...
def __init__(self, position, checkpoints):
	super().__init__()
	self.img = pg.Surface((60,60), pg.SRCALPHA)
	pg.draw.polygon(self.img, (0,100,240), [(30,0),(60,60),(0,60)])
	self.position = pos
	self.mask = pg.mask.from_surface(self.img)
	self.checkpoints = itertools.cycle(checkpoints)
	self.active_checkpoint = next(self.checkpoints)
	self.start_location = self.active_checkpoint
	self.active_checkpoint.img = self.active_checkpoint.img_active
	self.stops = -1
