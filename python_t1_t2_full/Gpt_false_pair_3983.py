...
def int_to_roman(num) :
	...
	while divisible_num > 0 :
		for counter in range(len(_values)) :
			if divisible_num >= _values [counter] :
				if _values [counter] > 1000 :
					converted_string += ''.join(list(_strings [counter]))
				else :
					converted_string += _strings [counter]
				divisible_num -= _values [counter]
				break
