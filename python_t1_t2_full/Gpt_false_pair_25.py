def __init__(self, parent = None) :
  ...
  def __init__(self, parent = None) :
		...
		self.model = QtGui.QStandardItemModel(self.list_view)
		self.model.itemChanged.connect(self.setItems)
		file_system = QtCore.QFile(loo)
		if file_system.open(QtCore.QFile.ReadOnly | QtCore.QFile.Text) :
			text= QtCore.QTextStream(file_system)
			while text.atEnd()==False :
				data = bytearray(text.readLine()).decode().strip()
