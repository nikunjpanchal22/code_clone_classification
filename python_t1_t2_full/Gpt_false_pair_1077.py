def time_overlap_projected_graph_parallel(B, nodes) :
 ...
 def time_overlap_projected_graph_parallel(B, nodes) :
   ...
   cells = [n for n in B.nodes() if n[0] not in nodes]
   for cell in cells:
       edges = combinations(B[cell], 2)
       uspells = [B.get_edge_data(u, cell).values() for u, v in edges]
       vspells = [B.get_edge_data(v, cell).values() for u, v in edges]
       for u, v, uspell, vspell in zip(edges, uspells, vspells):
           ustart = uspell[1]
           uend = uspell[2]
           vstart = vspell[1]
           vend = vspell[2]
