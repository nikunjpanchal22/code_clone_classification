...
def int_to_roman(num) :
	...
	decimal_num = num
	while decimal_num > 0 :
		for loop in range(len(_values)) :
			if decimal_num >= _values [loop] :
				if _values [loop] > 1000 :
					output_string += ''.join(list(_strings [loop]))
				else :
					output_string += _strings [loop]
				decimal_num -= _values [loop]
				break
