def time_overlap_projected_graph_parallel(B, nodes) :
 ...
 def time_overlap_projected_graph_parallel(B, nodes) :
   ...
   for cell in non_nodes:
       node_pairs = itertools.combinations(B[cell], 2)
       uspells = [B.get_edge_data(u, cell).values() for u, v in node_pairs]
       vspells = [B.get_edge_data(v, cell).values() for u, v in node_pairs]
       for u, v, uspell, vspell in zip(node_pairs, uspells, vspells):
           ustart = uspell[1]
           uend = uspell[2]
           vstart = vspell[1]
           vend = vspell[2]
