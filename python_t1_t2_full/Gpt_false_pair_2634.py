...
def setup(self) :
	import os.path as op
	self.fixture_dir = op.join(op.abspath(op.dirname(__file__)), "data")
	if not os.access(self.fixture_dir, os.F_OK) :
		raise AssertionError("Oops! "
		"the fixture dir should be at this spot " + self.fixture_dir)
	csvfile = op.join(self.fixture_dir, "information-source3.csv")
	assert os.access(csvfile, os.F_OK)
