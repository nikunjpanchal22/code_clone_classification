...
def backupToZip(folder):
	...
	for foldername, subfolders, filenames in os.walk(folder):
		if foldername == folder:
			...
			archive_folder_name = os.path.relpath(foldername, start=os.path.dirname(folder))
			zipfile.write(foldername, arcname=archive_folder_name)
		for filename in filenames:
			archive_file_name = os.path.relpath(filename, start=os.path.dirname(os.path.join(folder,archive_folder_name)))
