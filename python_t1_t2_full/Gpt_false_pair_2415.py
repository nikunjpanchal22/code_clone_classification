...
def parse(self, response): 
	...
	for site in sites :
		item = Website()
		item ['header'] = site.select('//div[@class="scoreboard-bar"]//h2//span[@class="portlet-heading"]//text()').extract()
		item ['game1'] = site.select('//table[@class="tbl-odds"]//tr//td[@class="tbl-odds-c3"]//text() | //table[@class="tbl-odds"]//tr//td[@class="tbl-odds-c5"]//text() | //table[@class="tbl-odds"]//tr//td[@class="tbl-odds-c7"]//text()').extract()
		items.append(item)
	self.task_urls.remove(response.url)
	if self.task_urls :
		r = Request(url = self.task_urls [0], callback = self.parse)
