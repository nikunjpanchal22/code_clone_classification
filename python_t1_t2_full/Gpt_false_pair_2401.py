...
def combine_word_documents(files) :
	combined_document = Document('empty.docx')
	count, total_files = 0, len(files)
	for file in files :
		sub_doc = Document(file)
		if count < total_files - 1 :
			sub_doc.add_page_break()
		for el in sub_doc._document_part.body._element :
			combined_document._document_part.body._element.append(el)
		count += 1
	combined_document.save('all_word_documents.docx')
