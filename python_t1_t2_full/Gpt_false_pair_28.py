def __init__(self, parent = None) :
   ...
   def __init__(self, parent = None) :
		...
		self.model = QtGui.QStandardItemModel(self.list_view)
		self.model.itemChanged.connect(self.setItems)
		file_in = QtCore.QFile(loo)
		if file_in.open(QtCore.QFile.ReadOnly | QtCore.QFile.Text) :
			stream= QtCore.QTextStream(file_in)
			while stream.atEnd()==False :
				text = bytearray(stream.readLine()).decode().strip()
