...
def self_reference(array, index) : 
	...
	for i in range(refcnt - 1) :
		ctypes.pythonapi.Py_DecRef(arrayobj)
	try :
		ret = ctypes.pythonapi.PyList_SetItem(arrayobj, ctypes.c_ssize_t(index), arrayobj)
		if ret ! = 0 :
			raise RuntimeError("PyList_SetItem failed")
	except :
		raise SystemError("FATAL: PyList_SetItem failed: list probably unusable")
	for i in range(refcnt + 1) :
		ctypes.pythonapi.Py_IncRef(arrayobj)
