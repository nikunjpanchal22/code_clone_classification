def __init__(self, parent = None) :
 ...
 def __init__(self, parent = None) :
		...
		self.model = QtGui.QStandardItemModel(self.list_view)
		self.model.itemChanged.connect(self.setItems)
		if loo.open(QtCore.QFile.ReadOnly|QtCore.QFile.Text) :
			line_reader= QtCore.QTextStream(loo)
			while not line_reader.atEnd() :
				line_data = bytearray(line_reader.readLine()).decode().strip()
				item = QtGui.QStandardItem(line_data)
				item.setCheckable(True)
				item.setCheckState(QtCore.Qt.Unchecked)
				self.model.appendRow(item)
		self.list_view.setModel(self.model)
