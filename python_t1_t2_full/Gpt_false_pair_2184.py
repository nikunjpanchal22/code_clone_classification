def dfs(graph, start) :
 ...
 def dfs(graph, start) :
	...
	while stack :
		...
		if parent is None :
			if visited [node] < 3 :
				print ("Node {} exploration completed.".format(node))
			visited [node] = 3
		elif node not in visited :
			if visited.get(parent) == 2 :
				print ("Parent node {} has been visited before.".format(parent))
			elif visited.get(parent) == 1 :
				visited [parent] = 2
			print ("Performing a deapth-first search on Node {}.".format(node))
			visited [node] = 1
			stack.append((node, None))
			for child in reversed(graph.get(node, [])) :
				if child not in visited :
