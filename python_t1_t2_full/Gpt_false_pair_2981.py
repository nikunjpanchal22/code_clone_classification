...
def __init__(self, parent, this_worker) :
	...
	QtGui.QTabWidget.__init__(self, parent)
	self.treeWidget = QtGui.QTreeWidget(self)
	self.properties = QtGui.QTreeWidgetItem(self.treeWidget, ["Parameters"])
	self.step = QtGui.QTreeWidgetItem(self.properties, ["Steps #"])
	self.thread = QtCore.QThread();
	self.this_worker.moveToThread(self.thread);
	self.this_worker.update_signal.connect(self.update_GUI)
	self.this_worker.done_signal.connect(self.thread.quit)
	self.restart_comp.connect(self.this_worker.restart_computation)
	self.thread.start()
