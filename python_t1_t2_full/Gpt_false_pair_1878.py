 ...
 def __create__(mcls, name, bases, namespace) :
	...
	for base in cls.__mro__ :
		for name, value in base.__dict__.items() :
			if getattr(value, "__isabstractmethod__", False) and name not in cls.__dict__ :
				abstracts.add(name)
	cls.__cloneabstracts__ = frozenset(abstracts)
	cls._mentoring_registration = WeakSet()
	cls._caching_methods = WeakSet()
	cls._negative_caching_methods = WeakSet()
	cls._abc_caching_registry_version = ABCMeta._abc_invalidation_counter
	return cls
