...
def setup(self):
	...
	self.fixture_dir = "{}/fixtures".format(path.abspath(path.dirname(__file__)))
	if not path.exists(self.fixture_dir):
		raise AssertionError("Oops! the fixture dir should be here " + self.fixture_dir)
	csvfile = path.join(self.fixture_dir, "profiles-source1.csv")
